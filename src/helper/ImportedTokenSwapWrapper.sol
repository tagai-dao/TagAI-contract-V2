...
import {IPoolManager as IUniswapV4PoolManager} from "v4-core/src/interfaces/IPoolManager.sol";
import {IUnlockCallback} from "v4-core/src/interfaces/callback/IUnlockCallback.sol";
import {IHooks as IUniswapV4Hooks} from "v4-core/src/interfaces/IHooks.sol";
import {PoolKey as UniswapV4PoolKey} from "v4-core/src/types/PoolKey.sol";
import {Currency as UniswapV4Currency} from "v4-core/src/types/Currency.sol";
import {
...
interface IImportedUniswapV3Router {
...
interface IImportedUniswapV3Quoter {
 ...
 *      Router02, Uniswap SwapRouter02-compatible V3 and Uniswap V4.
 ...
 */
contract ImportedTokenSwapWrapper is Ownable2Step, Pausable, ReentrancyGuard, IUnlockCallback {
    ...
    struct UniswapV4Source {
        INutboxRouter.UniswapV4Source pool;
    }
    ...
    function unlockCallback(bytes calldata data) external override returns (bytes memory) {
        ...
        (INutboxRouter.UniswapV4Source memory source, bool zeroForOne, uint256 amountIn) =
            abi.decode(data, (INutboxRouter.UniswapV4Source, bool, uint256));
        if (source.poolManager != msg.sender || !nutboxRouter.allowedUniswapV4Manager(msg.sender)) {
    ...
    function _quoteFirstHop(
        ...
        if (tokenIn == address(0) && sourceType != INutboxRouter.SourceType.UNISWAP_V4) {
            revert UnsupportedSwapSource();
        }
        if (sourceType == INutboxRouter.SourceType.V2_PAIR) {
            V2Source memory source = abi.decode(sourceData, (V2Source));
            ...
            V3Source memory source = abi.decode(sourceData, (V3Source));
            (amountOut,,,) = IImportedUniswapV3Quoter(source.quoter)
                .quoteExactInputSingle(
                    IImportedUniswapV3Quoter.QuoteExactInputSingleParams({
                    tokenIn: tokenIn,
                    tokenOut: tokenOut,
                    amountIn: amountIn,
            ...
            UniswapV4Source memory source = abi.decode(sourceData, (UniswapV4Source));
            amountOut =
                NutboxSpotPrice.quote(nutboxRouter, tokenIn, tokenOut, amountIn, sourceType, abi.encode(source.pool));
        } else {
            revert UnsupportedSwapSource();
        }
    ...
    function _executeFirstHop(
        ...
        if (sourceType == INutboxRouter.SourceType.V2_PAIR) {
            ...
            amountOut = _swapUniswapV4(tokenIn, tokenOut, amountIn, sourceData);
        } else {
            revert UnsupportedSwapSource();
        }
        ...
        returns (uint256 amountOut)
    {
        ...
        uint256 balanceBefore = outputToken.balanceOf(address(this));
        IERC20(tokenIn).forceApprove(source.router, amountIn);
        IImportedUniswapV3Router.ExactInputSingleParams memory params = IImportedUniswapV3Router.ExactInputSingleParams({
            tokenIn: tokenIn,
            tokenOut: tokenOut,
            fee: fee,
        ...
        IImportedUniswapV3Router(source.router).exactInputSingle(params);
        IERC20(tokenIn).forceApprove(source.router, 0);
        amountOut = outputToken.balanceOf(address(this)) - balanceBefore;
        if (amountOut == 0) revert InvalidSwapOutput();
    }
        ...
        returns (uint256 amountOut)
    {
        ...
        UniswapV4Source memory configuredSource = abi.decode(sourceData, (UniswapV4Source));
        INutboxRouter.UniswapV4Source memory source = configuredSource.pool;
        if (!nutboxRouter.allowedUniswapV4Manager(source.poolManager)) revert UnsupportedSwapSource();
        ...
        amountOut = abi.decode(manager.unlock(callbackData), (uint256));
    ...
    function _executeUniswapV4Swap(
        ...
        UniswapV4PoolKey memory key = _uniswapV4PoolKey(source);
    ...
    function _uniswapV4PoolKey(INutboxRouter.UniswapV4Source memory source)
        internal
        pure
