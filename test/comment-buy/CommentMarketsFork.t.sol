...
import {CommentTradeVault} from "../../src/autopay/CommentTradeVault.sol";
...
contract CommentMarketsForkTest is Test {
    ...
    address constant LEGACY = 0x4cA57c64DFe1cF1be977093C75f9d9cdd1DD2E10;
    address constant IMPORTED = 0xDfFc699FB095a693708E3c15e6F0a224cbbCc98F;
    address constant SHARES = 0x95450AaD4Cc195e03BB4791B7f6f04aC6D9BA922;
    ...
    function setUp() public {
        ...
        adapter = new CommentBuyAdapter(ROUTER, HOOK);
        vm.deal(address(this), 1 ether);
        ready = true;
    }
    modifier onFork() { if (!ready) vm.skip(true); _; }
    function _buy(address token, bytes memory plan) internal {
        (uint256 gross,,,) = adapter.quoteInput(token, 0.0001 ether);
        ...
        vault.setAllowedToken(token, true); vault.setPaused(false);
        vm.deal(recipient, 1 ether);
        vm.startPrank(recipient);
        vault.deposit{value: 0.1 ether}(0.01 ether);
        vault.authorize(0.1 ether, 0.01 ether, 0.05 ether, 0, 300, 100, block.timestamp + 1 days);
        vm.stopPrank();
        ...
        uint256 quote = adapter.buy{value: gross}(token, recipient, SUBJECT, 1, block.timestamp + 60, plan);
        vm.revertToState(snapshot);
        CommentTradeVault.Order memory order = CommentTradeVault.Order(bytes32(uint256(1)), recipient, token, SUBJECT,
            0.0001 ether, 0, 0, (quote * 9900 + 9999) / 10000, block.timestamp + 60, 1, quote, gross - 0.0001 ether);
    ...
    function _legacy(address token) internal {
        adapter.configureLegacy(token, LEGACY, SHARES, WBNB);
        (,,bytes32 hash) = adapter.routeContext(token);
        _buy(token, abi.encode(hash));
    }
    function testForkBUIDL() public onFork { _legacy(0x32ef878D527d860339818571E8DA17005110f04E); }
    function testForkTTAI() public onFork { _legacy(0x8e11E90B463bf521382E2B88539F053270a3848c); }
    function testForkTagClaw() public onFork { _legacy(0xe7324F2987aCd88Ee7286EB9DAb0EE926ad36a68); }
    function testForkCyberCab() public onFork {
        (uint256 gross,,,) = adapter.quoteInput(CYBER, 0.0001 ether);
    ...
    function testForkImportedV3UsdtQuote() public onFork {
        address token = 0xBEEA1D618e533a387D941F58a7d4c9b7bD377777;
        adapter.configureImported(token, IMPORTED, 1, abi.encode(
