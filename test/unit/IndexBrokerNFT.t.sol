...
contract IndexBrokerCommunityToken is ERC20 {
    ...
    /// @dev RH Token 的 Uniswap V4 PoolManager（与 clPoolManager 同址，保留旧字段供旧用例）。
    ...
    /// @dev RH Token 上市池的 tick spacing（与 Token.sol 常量一致）。
    ...
    function _configureListing(address manager, address hook, bytes32 parameters, address quoteToken, bool listed_)
        internal
    {
        ...
        // Uniswap V4 PoolKey.toId() = keccak256(abi.encode(currency0, currency1, fee, tickSpacing, hooks))（5 字段）。
        v4PoolId = keccak256(abi.encode(currency0, currency1, LISTING_LP_FEE, TICK_SPACING, hook));
        IndexBrokerPancakeV4CLManagerMock(manager)
...
contract IndexBrokerPancakeV4CLManagerMock {
    ...
    // ---- Uniswap V4 StateLibrary 适配 ----
    // NutboxSpotPrice 通过 StateLibrary（extsload）读取 slot0 / liquidity，
    // 这里按 v4-core 的存储布局写入对应 slot，使官方池定价可读。
    bytes32 internal constant _POOLS_SLOT = bytes32(uint256(6));
    uint256 internal constant _LIQUIDITY_OFFSET = 3;
    mapping(bytes32 => bytes32) private _v4Storage;

    function _poolStateSlot(bytes32 poolId) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(poolId, _POOLS_SLOT));
    }

    function setSlot0V4(bytes32 poolId, uint160 sqrtPriceX96, int24 tick, uint24 protocolFee, uint24 lpFee) external {
        bytes32 packed = bytes32(
            uint256(sqrtPriceX96) | (uint256(uint24(tick)) << 160) | (uint256(protocolFee) << 184)
                | (uint256(lpFee) << 208)
        );
        _v4Storage[_poolStateSlot(poolId)] = packed;
    }

    function setLiquidityV4(bytes32 poolId, uint128 liquidity) external {
        _v4Storage[bytes32(uint256(_poolStateSlot(poolId)) + _LIQUIDITY_OFFSET)] = bytes32(uint256(liquidity));
    }

    function extsload(bytes32 slot) external view returns (bytes32) {
        return _v4Storage[slot];
    }

    function extsload(bytes32 slot, uint256 n) external view returns (bytes32[] memory) {
        bytes32[] memory out = new bytes32[](n);
        for (uint256 i; i < n; ++i) {
            out[i] = _v4Storage[bytes32(uint256(slot) + i)];
        }
        return out;
    }
}
...
contract IndexBrokerNFTTest is Test {
    ...
    function setUp() public {
        ...
        // RH：官方上市池走 Uniswap V4，manager 注册到 uniswapV4Managers；Pancake V4 CL allowlist 留空。
        address[] memory uniswapV4Managers = new address[](1);
        uniswapV4Managers[0] = address(pancakeV4Manager);
    ...
    function test_UnlistedOfficialTokenActivatesAfterListingOnlyOnce() public {
        ...
        // Uniswap V4：用 StateLibrary 存储布局写入 slot0 / liquidity，供 NutboxSpotPrice 读取。
        pancakeV4Manager.setSlot0V4(officialToken.v4PoolId(), uint160(1 << 96), 0, 0, officialToken.LISTING_LP_FEE());
        pancakeV4Manager.setLiquidityV4(officialToken.v4PoolId(), 1_000_000);

        vm.prank(paidUser);
        officialAMM.activate();

        assertTrue(officialAMM.active());
        assertEq(uint8(officialAMM.priceSourceType()), uint8(INutboxRouter.SourceType.UNISWAP_V4));
        INutboxRouter.UniswapV4Source memory source =
            abi.decode(officialAMM.priceSourceData(), (INutboxRouter.UniswapV4Source));
        ...
        assertEq(source.tickSpacing, officialToken.TICK_SPACING());
    ...
    function test_AlreadyListedOfficialTokenCreatesImmediatelyActiveAMM() public {
        ...
        for (uint256 i; i < logs.length; ++i) {
            ...
            assertEq(emittedSourceType, uint8(INutboxRouter.SourceType.UNISWAP_V4));
