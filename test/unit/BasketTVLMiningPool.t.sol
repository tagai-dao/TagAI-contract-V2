...
contract BasketTVLTestBasket is ERC20 {
    ...
    constructor(string memory name_, string memory symbol_, address creator_, address wbnb_, address constituent_)
        ERC20(name_, symbol_)
    {
    ...
    function injectHolderFees(uint256 amount) external {
        BasketTVLTestERC20(wbnb).transferFrom(msg.sender, address(this), amount);
    ...
    function claimHolderFeesFor(address holder) external returns (uint256 amount) {
        ...
        BasketTVLTestERC20(wbnb).transfer(holder, amount);
    }
...
contract BasketTVLTestMultiAssetBasket is ERC20 {
    ...
    constructor(address creator_, address wbnb_, address[] memory assets_) ERC20("Multi Asset Basket", "MAB") {
        creatorPayout = creator_;
        wbnb = wbnb_;
        _assets = assets_;
    }
