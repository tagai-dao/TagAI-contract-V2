...
contract ImportHelperTest is Test {
    ...
    function test_createCommunityAndPool_revertsInsufficientFeeWithoutIPShare() public {
        uint256 nutboxFees = CREATE_COMMUNITY_FEE + SETTINGS_FEE;
        ...
        helper.createCommunityAndPool{value: totalRequired - 1}(
            address(token),
            address(calculator),
            bytes("")
        );
    ...
    function test_createCommunityAndPool_createsIPShareAndRecordsImporter() public {
        ...
        vm.prank(creator, creator);
        (address community, address pool) = helper.createCommunityAndPool{value: totalRequired + dust}(
            address(token),
            address(calculator),
            bytes("")
        );
