...
import {ILockCallback as ImportedTestLockCallback} from "infinity-core/src/interfaces/ILockCallback.sol";
import {
    ICLPoolManager as ImportedTestPancakeV4CLPoolManager
} from "infinity-core/src/pool-cl/interfaces/ICLPoolManager.sol";
import {PoolKey as ImportedTestPancakeV4PoolKey} from "infinity-core/src/types/PoolKey.sol";
import {Currency as ImportedTestPancakeV4Currency} from "infinity-core/src/types/Currency.sol";
import {
    BalanceDelta as ImportedTestPancakeV4BalanceDelta,
    toBalanceDelta as toImportedTestPancakeV4BalanceDelta
...
contract ImportedV3QuoterMock {
    function quoteExactInputSingle(IImportedPancakeV3Quoter.QuoteExactInputSingleParams calldata params)
        external
        pure
...
contract ImportedPancakeV4QuoterMock {
    function quoteExactInputSingle(IImportedPancakeV4Quoter.QuoteExactInputSingleParams calldata params)
        external
        pure
        returns (uint256 amountOut, uint256 gasEstimate)
    {
        return (uint256(params.exactAmount) * 2, 0);
    }
}

contract ImportedPancakeV4VaultMock {
    address private _locker;

    function lock(bytes calldata data) external returns (bytes memory result) {
        require(_locker == address(0), "already locked");
        _locker = msg.sender;
        result = ImportedTestLockCallback(msg.sender).lockAcquired(data);
        _locker = address(0);
    }
    ...
    function sync(ImportedTestPancakeV4Currency) external {}

    function settle() external payable returns (uint256) {
        return msg.value;
    }

    function take(ImportedTestPancakeV4Currency currency, address recipient, uint256 amount) external {
        address token = ImportedTestPancakeV4Currency.unwrap(currency);
        if (token == address(0)) {
...
contract ImportedPancakeV4ManagerMock {
    address public immutable vault;
    mapping(bytes32 => uint160) private _prices;
    mapping(bytes32 => uint128) private _liquidities;
    ...
    function setPool(bytes32 poolId, uint160 sqrtPriceX96, uint128 liquidity) external {
        _prices[poolId] = sqrtPriceX96;
        _liquidities[poolId] = liquidity;
    }

    function getSlot0(bytes32 poolId) external view returns (uint160, int24, uint24, uint24) {
        return (_prices[poolId], 0, 0, 0);
    }

    function getLiquidity(bytes32 poolId) external view returns (uint128) {
        return _liquidities[poolId];
    }

    function swap(
        ImportedTestPancakeV4PoolKey calldata,
        ImportedTestPancakeV4CLPoolManager.SwapParams calldata params,
        bytes calldata
    ) external pure returns (ImportedTestPancakeV4BalanceDelta delta) {
        uint256 amountIn = uint256(-params.amountSpecified);
        int128 signedIn = int128(int256(amountIn));
        int128 signedOut = int128(int256(amountIn * 2));
        return params.zeroForOne
            ? toImportedTestPancakeV4BalanceDelta(-signedIn, signedOut)
            : toImportedTestPancakeV4BalanceDelta(signedOut, -signedIn);
    }
}
...
contract ImportedCoreRouterMock {
    ...
    function setPancakeV4(address manager, bool allowed) external {
        allowedPancakeV4CLManager[manager] = allowed;
    }
...
contract ImportedTokenSwapWrapperTest is Test {
    ...
    ImportedPancakeV4QuoterMock private v4Quoter;
    ...
    function setUp() public {
        ...
        v4Quoter = new ImportedPancakeV4QuoterMock();
    ...
    function test_buyPancakeV4WithCallerSuppliedNonNativeQuote() public {
        ImportedPancakeV4VaultMock vault = new ImportedPancakeV4VaultMock();
        ImportedPancakeV4ManagerMock manager = new ImportedPancakeV4ManagerMock(address(vault));
        coreRouter.setPancakeV4(address(manager), true);
        ...
        assertEq(
            wrapper.quoteBuy(address(token), INutboxRouter.SourceType.PANCAKE_V4_CL, sourceData, 1 ether), 1.992 ether
        );
        assertEq(
            wrapper.quoteSell(address(token), INutboxRouter.SourceType.PANCAKE_V4_CL, sourceData, 10 ether), 19.92 ether
        );

        vm.prank(buyer);
        uint256 tokenOut = wrapper.buyToken{value: 1 ether}(
            address(token),
            INutboxRouter.SourceType.PANCAKE_V4_CL,
            sourceData,
            1.992 ether,
            buyer,
            block.timestamp + 1,
    ...
    function test_buyAndSellPancakeV4WithNativeQuoteSkipsCoreRouter() public {
        ImportedPancakeV4VaultMock vault = new ImportedPancakeV4VaultMock();
        ImportedPancakeV4ManagerMock manager = new ImportedPancakeV4ManagerMock(address(vault));
        coreRouter.setPancakeV4(address(manager), true);
    ...
    function test_pancakeV4NonzeroHookNeedsNoRegistration() public {
        ImportedPancakeV4VaultMock vault = new ImportedPancakeV4VaultMock();
        ImportedPancakeV4ManagerMock manager = new ImportedPancakeV4ManagerMock(address(vault));
        coreRouter.setPancakeV4(address(manager), true);
    ...
    function test_uniswapV4SourceIsNotSupportedOnBscV1() public {
        vm.expectRevert(ImportedTokenSwapWrapper.UnsupportedSwapSource.selector);
        wrapper.resolveQuoteToken(address(token), INutboxRouter.SourceType.UNISWAP_V4, "");
    }
    ...
    function _v4Source(INutboxRouter.PancakeV4CLSource memory source) private view returns (bytes memory sourceData) {
        sourceData = abi.encode(ImportedTokenSwapWrapper.PancakeV4Source({quoter: address(v4Quoter), pool: source}));
    }
