...
contract RHBasketTVLMiningPoolForkTest is Test {
    ...
    function setUp() public {
        ...
        community = Community(
            payable(
                communityFactory.createCommunity(
        ...
        ratios[0] = 10_000;
        community.adminAddPool("RH Live Basket TVL Mining", ratios, address(factory), bytes(""));
    ...
    function testFork_CreateBothStakesUsesLiveNavAndConfiguresChildren() public onlyRhFork {
        ...
        vm.prank(keeper);
        BasketStakePool childA = BasketStakePool(pool.createBasketStake(LIVE_BASKET_A));
        vm.prank(keeper);
        BasketStakePool childB = BasketStakePool(pool.createBasketStake(LIVE_BASKET_B));
    ...
    function testFork_PermissionlessUpdateRequotesAllLiveAssets() public onlyRhFork {
        BasketStakePool child = BasketStakePool(pool.createBasketStake(LIVE_BASKET_A));
    ...
    function testFork_RealBasketHolderCanDepositWithdrawAndRedeem() public onlyRhFork {
        BasketStakePool child = BasketStakePool(pool.createBasketStake(LIVE_BASKET_B));
    ...
    function testFork_RejectsInvalidAndDuplicateLiveBasketRegistration() public onlyRhFork {
        ...
        pool.createBasketStake(makeAddr("notARegisteredBasket"));

        pool.createBasketStake(LIVE_BASKET_A);
    ...
    function testFork_ClosedParentBlocksNewBasketAndChildDeposits() public onlyRhFork {
        ...
        pool.createBasketStake(LIVE_BASKET_B);
    ...
    function testFork_TwoRealBasketHoldersSplitCommunityRewardsByStake() public onlyRhFork {
        ...
        uint256 expectedAlice = Math.mulDiv(ONE_HOUR_REWARD, aliceAmount, aliceAmount + bobAmount);
        uint256 expectedBob = ONE_HOUR_REWARD - expectedAlice;
        assertApproxEqAbs(aliceReward, expectedAlice, 1e8);
        assertApproxEqAbs(bobReward, expectedBob, 1e8);
        assertApproxEqAbs(aliceReward + bobReward, ONE_HOUR_REWARD, 1e8);
    ...
    function testFork_CommunityRewardsSplitAcrossBothLiveBasketNavs() public onlyRhFork {
        ...
        uint256 expectedA = Math.mulDiv(ONE_HOUR_REWARD, navA, navA + navB);
        uint256 expectedB = ONE_HOUR_REWARD - expectedA;
        assertApproxEqAbs(rewardA, expectedA, 1e8);
        assertApproxEqAbs(rewardB, expectedB, 1e8);
        assertApproxEqAbs(rewardA + rewardB, ONE_HOUR_REWARD, 1e8);
    }
    ...
    function testFork_RealBasketFeeAccrualExternalClaimAndNavRefresh() public onlyRhFork {
        ...
        assertApproxEqAbs(communityReward, ONE_HOUR_REWARD, 1e8);
    ...
    function testFork_OperationFeeUnderpaymentAndExcessRefundWithRealBasket() public onlyRhFork {
        ...
        assertApproxEqAbs(reward, ONE_HOUR_REWARD, 1e8);
    ...
    function testFork_ClosedParentAllowsExactlyOneFinalRealBasketHarvest() public onlyRhFork {
        ...
        assertApproxEqAbs(finalReward, ONE_HOUR_REWARD, 1e8);
    ...
    function _assertCreatedStake(address basket, BasketStakePool child, uint256 expectedNav) internal view {
        ...
        assertEq(stake.owner, IBasketToken(basket).creatorPayout());
    ...
    function _mintThroughRealBasketEngine(address basket, uint256 sharesOut, uint256 feeWeth) internal {
        ...
        vm.prank(engine);
        uint256 minted = token.engineMintFromAcquired(
            keeper, requirements, 0, feeWeth, address(0)
        );
        assertGt(minted, 0);
    }
}
